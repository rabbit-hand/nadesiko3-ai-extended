...
const PluginScienceExperiments = {
  ...
  // --- 基礎科学実験 ---
  '簡単実験リスト': { // @簡単な科学実験リスト // @かんたんじっけんりすと
    type: 'func',
    josi: [],
    pure: true,
    fn: function (): any[] {
  ...
  '実験手順': { // @実験の手順を取得 // @じっけんてじゅん
    type: 'func',
    josi: [['の']],
    pure: true,
    fn: function (experimentName: string): any {
  ...
  '振り子計算': { // @振り子の周期を計算 // @ふりこけいさん
    type: 'func',
    josi: [['の'], ['で']],
    pure: true,
    fn: function (length: number, gravity: number = 9.8): any {
      if (length <= 0) return { error: '長さは正の値が必要です' }
  ...
  '光の屈折計算': { // @光の屈折角を計算 // @ひかりのくっせつけいさん
    type: 'func',
    josi: [['で'], ['から'], ['に']],
    pure: true,
    fn: function (incidentAngle: number, n1: number, n2: number): any {
  ...
  'pH計算': { // @pH値を計算 // @pHけいさん
    ...
    josi: [['の']],
    pure: true,
    fn: function (hConcentration: number): any {
      if (hConcentration <= 0) return { error: '水素イオン濃度は正の値が必要です' }
      
      const pH = -Math.log10(hConcentration)
      let nature = ''
      
      if (pH < 7) nature = '酸性'
      else if (pH === 7) nature = '中性'
      else nature = 'アルカリ性'
  ...
  '化学反応シミュレーション': { // @化学反応をシミュレーション // @かがくはんのうしみゅれーしょん
    type: 'func',
    josi: [['と'], ['を']],
    pure: true,
    fn: function (reactant1: string, reactant2: string): any {
      const reactions: any = {
        'H2+O2': {
          product: 'H2O',
          reactionType: '化合',
  ...
  '細胞計算': { // @細胞の計算 // @さいぼうけいさん
    ...
    josi: [['を'], ['で']],
    pure: true,
    fn: function (initialCells: number, generations: number): any {
      ...
      const totalCells = Array.from({ length: generations + 1 }, (_, i) => 
        initialCells * Math.pow(2, i)
  ...
  '遺伝子計算': { // @メンデル遺伝の計算 // @いでんしけいさん
    type: 'func',
    josi: [['と']],
    pure: true,
    fn: function (parent1: string, parent2: string): any {
      // 簡単なメンデル遺伝（優性・劣性）
      const dominant = 'A'
      const recessive = 'a'
  ...
  '地質年代計算': { // @地質年代を計算 // @ちしつねんだいけいさん
    type: 'func',
    josi: [['で'], ['を']],
    pure: true,
    fn: function (halfLife: number, remainingRatio: number): any {
  ...
  '円周率計算': { // @円周率を計算 // @えんしゅうりつけいさん
    type: 'func',
    josi: [['の'], ['を']],
    pure: true,
    fn: function (diameter: number, circumference: number): any {
  ...
  '黄金比計算': { // @黄金比を計算 // @おうごんひけいさん
    type: 'func',
    josi: [['と']],
    pure: true,
    fn: function (longer: number, shorter: number): any {
  ...
  '惑星計算': { // @惑星の軌道計算 // @わくせいきどうけいさん
    type: 'func',
    josi: [['の'], ['で']],
    pure: true,
    fn: function (semiMajorAxis: number, eccentricity: number): any {
  ...
  // --- 環境科学実験 ---
  '環境実験リスト': { // @環境科学実験リスト // @かんきょうかがくじっけんりすと
    type: 'func',
    josi: [],
    pure: true,
    fn: function (): any[] {
  ...
  '環境指数計算': { // @環境汚染指数を計算 // @かんきょうおせんしすうけいさん
    type: 'func',
    josi: [['の']],
    pure: true,
    fn: function (pollutantValues: any): any {
      ...
      Object.keys(weights).forEach(pollutant => {
        if (pollutantValues[pollutant]) {
          index += (pollutantValues as any)[pollutant] * (weights as any)[pollutant]
          totalWeight += (weights as any)[pollutant]
        }
      })
      ...
      let level = '良好'
      if (finalIndex > 100) level = '重度汚染'
      else if (finalIndex > 50) level = '中度汚染'
      else if (finalIndex > 25) level = '軽度汚染'
  ...
  '単位変換': { // @科学単位の変換 // @たんいへんかん
    type: 'func',
    josi: [['を'], ['から'], ['に']],
    pure: true,
    fn: function (value: number, fromUnit: string, toUnit: string): any {
