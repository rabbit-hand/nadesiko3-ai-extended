...
const PluginBeginnerSupport = {
  ...
  // --- 自然言語プログラミング ---
  '自然言語変換': { // @自然言語をプログラムに変換 // @しぜんげんごへんかん
    type: 'func',
    josi: [['を']],
    pure: true,
    fn: function (naturalText: string): any {
      ...
      const patterns = [
        {
          pattern: /(.+)を表示して/,
          action: '表示',
          extract: (match: string[]) => ({ content: match[1] })
        },
        {
          pattern: /(.+)を出力して/,
          action: '表示',
        ...
        },
        {
          ...
          action: '繰り返し',
          extract: (match: string[]) => ({ count: parseInt(match[1]) })
        },
        {
          pattern: /もし(.+)なら|もし(.+)だったら/,
          action: '条件',
          extract: (match: string[]) => ({ condition: match[1] === 'true' || match[1] === '真' })
        },
        {
          pattern: /(.+)を入力して|(.+)を尋ねて/,
          action: '入力',
          extract: (match: string[]) => ({ prompt: match[1] })
        },
        {
  ...
  '自然言語実行': { // @自然言語プログラムを実行 // @しぜんげんごじっこう
    type: 'func',
    josi: [['を']],
    pure: true,
    fn: function (naturalText: string): any {
      ...
      let result = { action: converted.action, output: null as any }
        ...
        switch (converted.action) {
          ...
          case '計算':
            if (converted.parameters.operation === 'add') {
              result.output = converted.parameters.values.reduce((sum: number, val: number) => sum + val, 0)
            } else if (converted.parameters.operation === 'multiply') {
              result.output = converted.parameters.values.reduce((prod: number, val: number) => prod * val, 1)
  ...
  // --- 対話型プログラミング支援 ---
  '対話開始': { // @対話型プログラミングを開始 // @たいわがたプログラミングかいし
    type: 'func',
    josi: [],
    pure: true,
    fn: function (): any {
  ...
  '対話応答': { // @対話型プログラミングで応答 // @たいわがたプログラミングおうとう
    type: 'func',
    josi: [['に'], ['を']],
    pure: true,
    fn: function (session: any, userInput: string): any {
  ...
  // --- テンプレート機能 ---
  'テンプレート一覧': { // @プログラミングテンプレート一覧 // @てんぷれーといちらん
    type: 'func',
    josi: [],
    pure: true,
    fn: function (): any[] {
  ...
  'テンプレート適用': { // @テンプレートを適用 // @てんぷれーとてきよう
    type: 'func',
    josi: [['を']],
    pure: true,
    fn: function (templateName: string): any {
  ...
  // --- エラー支援 ---
  'エラー解決案': { // @プログラミングエラーの解決案 // @えらーかいけつあん
    type: 'func',
    josi: [['の']],
    pure: true,
    fn: function (errorMessage: string): any {
      const errorSolutions: any = {
        '未解決の単語': {
  ...
  // --- 学習支援 ---
  '学習パス生成': { // @個別学習パスを生成 // @がくしゅうぱすせいせい
    type: 'func',
    josi: [['の'], ['で']],
    pure: true,
    fn: function (level: string, interest: string): any {
      const learningPaths = {
        '初心者': {
          'ゲーム': [
      ...
      const path = (learningPaths as any)[level]?.[interest] || []
      ...
      return {
        ...
        interest: interest,
        path: path,
        totalTime: path.reduce((sum: any, step: any) => sum + step.time, 0),
        estimatedDays: Math.ceil(path.reduce((sum: any, step: any) => sum + step.time, 0) / 30),
        difficulty: '段階的',
        timestamp: Date.now()
      }
    }
  ...
  '進捗確認': { // @学習進捗を確認 // @しんちょかくにん
    type: 'func',
    josi: [['の']],
    pure: true,
    fn: function (progressData: any): any {
      ...
      let level = '初心者'
      if (completionRate > 80) level = '上級者'
      else if (completionRate > 50) level = '中級者'
  ...
  // --- アクセシビリティ機能 ---
  '音声読み上げ': { // @コードを音声で読み上げ // @おんせいよみあげ
    type: 'func',
    josi: [['を']],
    pure: true,
    fn: function (text: string): any {
      if (!text) return { error: 'テキストが必要です' }
  ...
  '文字サイズ調整': { // @文字サイズを調整 // @もじさいずちょうせい
    type: 'func',
    josi: [['に']],
    pure: true,
    fn: function (size: string): any {
      const sizes = {
      ...
      const setting = (sizes as any)[size] || sizes['普通']
  ...
  // --- 簡単化機能 ---
  'プログラム簡素化': { // @プログラムを簡単にする // @ぷろぐらむかんたんか
    type: 'func',
    josi: [['を']],
    pure: true,
    fn: function (complexCode: string): any {
      if (!complexCode) return { error: 'コードが必要です' }
