...
- **AI魔改造品**: これはAIで魔改造したものです！Pythonレベルの高度な機能（データサイエンス、機械学習、深層学習、自然言語処理、時系列分析、IoT制御、Web API連携）を完全に実装した究極の日本語プログラミング言語です。教育・研究・実務で活用できるAI搭載の最強なでしこ3！
...
- **データサイエンス拡張**: 以下のプラグインでPythonライクなデータサイエンス機能を実装
  - `src/plugin_datascience.mjs` - データ処理（NumPy風機能）
    - 配列操作、数学関数、統計関数（平均、標準偏差、相関係数など）
  - `src/plugin_statistics.mjs` - 統計分析
    - 記述統計、確率分布、仮説検定、回帰分析
  - `src/plugin_machinelearning.mjs` - 機械学習
    - データ前処理、教師あり学習、教師なし学習、評価指標
...
- Lint: `npm run eslint`
- **データサイエンス関連**:
  - データサイエンスプラグインテスト: `npm run test:datascience`
  - デモ実行: `npm run demo:datascience`
  - 簡単テスト: `npm run demo:datascience:simple`
  - パッケージ作成: `npm run package:datascience`
...
- **データサイエンスプラグイン変更時**: `npm run test:datascience` を実行し、`npm run demo:datascience` で動作確認
...
- **データサイエンス機能**: `README_DATASCIENCE.md`
